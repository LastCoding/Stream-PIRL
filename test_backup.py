...
class RangeBar(GraphicsPort):
    ...
    def __external_data_provider(self, cell):
        varr = int(self.__external_data_adapter.get_data(cell))
        print(varr)
        return varr
    ...
    def execute_action(self, action, **kwargs):
        ...
        match action:
            ...
            case "external_data_provider":
                return self.__external_data_provider(kwargs["cell"])
...
def main():
    ...
    baseCell = rb.execute_action("external_data_provider", cell="A1")
    rb.setValue(baseCell)
    while True:
        changeCell = rb.execute_action("external_data_provider", cell="B1")
        rb.execute_action("decrement",number=changeCell)
        svg = rb.render()
